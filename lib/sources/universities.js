...
async function fetchYTUFullDate(url) {
  try {
    const res = await axios.get(url, { headers: HEADERS, timeout: 10000 });
    const $ = cheerio.load(res.data);
    // YTÜ detay sayfasında tarih genellikle .event-date, .tarih, time, veya ilk paragrafta
    const candidates = [
      $(".event-date, .etkinlik-tarih, .date, time").first().text().trim(),
      $("p").first().text().trim(),
      $("h2, h3").eq(1).text().trim(),
    ];
    for (const c of candidates) {
      // "15 Nisan 2026" veya "15.04.2026" gibi yıl içeren format ara
      const m = c.match(/\d{1,2}\s+\w+\s+20\d{2}|\d{1,2}\.\d{1,2}\.20\d{2}|20\d{2}-\d{2}-\d{2}/);
      if (m) return m[0];
    }
  } catch {}
  return null;
  ...
  for (const { url, type } of pages) {
    try {
      ...
      if (type === "ytu-list") {
        ...
        // YTÜ için detay sayfasından tam tarih çek
        for (const e of events) {
          if (e.date && !/20\d{2}/.test(e.date)) {
            const fullDate = await fetchYTUFullDate(e.url);
            if (fullDate) e.date = fullDate;
          }
          await sleep(300);
        }
      } else {
