...
function parseEventDate(dateStr) {
  ...
  if (trMatch) return new Date(parseInt(trMatch[3]), TR_MONTHS[trMatch[2]], parseInt(trMatch[1]));
...
function isNotPast(event) {
  ...
  if (!date) {
    // URL'de eski yıl varsa atla
    const urlYear = (event.url || "").match(/\/(201\d|202[0-4])\//);
