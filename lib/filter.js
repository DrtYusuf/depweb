...
function parseEventDate(dateStr) {
  ...
  // "15 nisan 2026" veya "15 nis 2026"
  const trMatch = s.match(/(\d{1,2})\s*(ocak|şubat|mart|nisan|mayıs|haziran|temmuz|ağustos|eylül|ekim|kasım|aralık|oca|şub|mar|nis|may|haz|tem|ağu|eyl|eki|kas|ara)\s*(\d{4})?/);
  if (trMatch) {
    const year = trMatch[3] ? parseInt(trMatch[3]) : new Date().getFullYear();
    return new Date(year, TR_MONTHS[trMatch[2]], parseInt(trMatch[1]));
  }
...
function isNotPast(event) {
  ...
  if (!date) {
    // URL'de geçmiş yıl varsa atla
    const urlYear = (event.url || "").match(/\/(20\d{2})\//);
    if (urlYear && parseInt(urlYear[1]) < today.getFullYear()) return false;
    return true;
  }
...
module.exports = { isRelevant, isBMERelevant, isIstanbulOrOnline, isNotPast, parseEventDate };
