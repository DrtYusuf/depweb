...
// Tarihte ay VE yıl açıkça belirtilmiş mi?
function hasExplicitMonthYear(dateStr) {
  if (!dateStr) return false;
  const s = dateStr.trim().toLowerCase();
  const trMonths = Object.keys(TR_MONTHS).join("|");
  const enMonths = Object.keys(EN_MONTHS).join("|");
  // "nisan 2026", "15 nisan 2026", "april 2026", "2026-04-15" vb.
  return (
    new RegExp(`(${trMonths})\\s+\\d{4}`).test(s) ||
    new RegExp(`\\d{1,2}\\s+(${trMonths})\\s+\\d{4}`).test(s) ||
    new RegExp(`(${enMonths})\\s+\\d{4}`).test(s) ||
    new RegExp(`\\d{1,2}\\s+(${enMonths})\\s+\\d{4}`).test(s) ||
    /\d{4}-\d{2}-\d{2}/.test(s) ||
    /\d{1,2}\.\d{1,2}\.\d{4}/.test(s)
  );
}

function isWithinSixMonths(event) {
  if (!hasExplicitMonthYear(event.date)) return false;
...
module.exports = { isRelevant, isActualEvent, isBMERelevant, isIstanbulOrOnline, isNotPast, isWithinSixMonths, parseEventDate };
