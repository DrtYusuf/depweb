...
function parseBestEffortDate(dateStr) {
  if (!dateStr) return null;
  const s = dateStr.trim().toLowerCase();

  // Yıllı tam format
  const full = parseEventDate(dateStr);
  if (full) return full;

  // "25Ara", "15Haz" gibi yılsız — bu yıla ata
  const trKeys = Object.keys(TR_MONTHS).join("|");
  const shortMatch = s.match(new RegExp(`^(\\d{1,2})\\s*(${trKeys})$`));
  if (shortMatch) {
    const year = new Date().getFullYear();
    return new Date(year, TR_MONTHS[shortMatch[2]], parseInt(shortMatch[1]));
  }
...
function isWithinSixMonths(event) {
  const date = parseBestEffortDate(event.date);
